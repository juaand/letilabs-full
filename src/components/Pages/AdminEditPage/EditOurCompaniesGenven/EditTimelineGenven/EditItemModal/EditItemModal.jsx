...
import {deleteTimelineGenven, updateGenvenTimeline, createContent} from '../../../../../../services/ApiClient'
...
function EditItemModal({deleteItem, infodata, hideModal, closeModal}) {
    ...
    const {state, onChange} = useFormState(
        {
            data: {
                ...
                buttonText: timelineData.buttonText,
    ...
    const updateInfo = async (event) => {
        ...
        if (Object.values(error).map(el => el).includes(false)) {
            try {
                await updateGenvenTimeline(data)
                    .then(info => {
    ...
    const deleteSelected = async (id) => {
        const updatedData = await deleteTimelineGenven(id)
